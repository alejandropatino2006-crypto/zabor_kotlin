...
@Component({
  selector: 'app-total-sales-by-shift',
  templateUrl: './total-sales-by-shift.component.html',
  styleUrls: ['./total-sales-by-shift.component.scss']
})
export class TotalSalesByShiftComponent implements OnInit {
  ...
  onShiftChange() {
    this.currentShift = this.shiftList.find((shift) => shift.id == this.shiftId);
    console.log("currentShift", this.currentShift);
    this.tableTitle = `Report for Shift = Start : ${this.currentShift.open_date_time} - End : ${this.currentShift.close_date_time}`;
  }
