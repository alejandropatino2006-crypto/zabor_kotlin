...
import {isErrorResponse, TerminalLocationRequest, TerminalReaderRequest} from '../../../shared/terminal/api-client';
...
export class TerminalComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(private http: HttpClient, private routingRouter: Router, private formBuilder: FormBuilder/*, private admService: adminService*/, private spinner: NgxSpinnerService, private httpOptionsService: HttpOptionsService) {
    ...
    if (this.useSimulatedTerminal) {
      ...
      if (error) {
        ...
        setTimeout(() => {
          ...
          this.routingRouter.navigate(['admin/terminal/test']);
        }, 500);
      }
      console.groupEnd();
    } else {
  ...
  private loadAccounts() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        that.http
          .get<DataTablesResponse>(`${environment.apiUrl}` + "/pmt/accounts-list", this.httpOptionsService.makeHttpRequestOptionsWithAuthentication())
          .subscribe(resp => {
