...
export class SalesByEmployeeComponent implements OnInit {
  ...
  inputChanged(evt: Object) {
    if (this.selectedDate && this.selectedDate.startDate !== undefined && this.selectedDate.startDate !== null && this.resId !== -1 && this.employeeId !== -1) {
      const mysqlStartDate = this.selectedDate.startDate.format("YYYY-MM-DD");

      this.shiftList = [];

      this.restaurantService.getAllShiftsByRestAndDateAndEmp(
        this.resId,
        mysqlStartDate,
        this.employeeId
      )
  ...
  onRestaurantChange() {
    ...
    .subscribe(
      ...
      (error) => {
        Swal.fire(Swaldata.SwalErrorToast("Employees List didnt come"));
      }
    )
    .add(() => {
      // this.spinner.hide();
  ...
  onEmployeeChange() {
    this.currentEmployee = this.employeeList.find((emp) => emp.id == this.employeeId);
    console.log("currentEmployee", this.currentEmployee);
    this.tableTitle = `Report for Employee = ${this.currentEmployee.name}`;
  }
  ...
  ngOnInit() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        dataTablesParameters['employeeId'] = this.employeeId;
        ...
        that.httpClient
          .post<DataTablesResponse>(`${environment.apiUrl}/user/getSalesReport`, dataTablesParameters, this.httpOptionsService.makeHttpRequestOptionsWithAuthentication())
          .subscribe(resp => {
