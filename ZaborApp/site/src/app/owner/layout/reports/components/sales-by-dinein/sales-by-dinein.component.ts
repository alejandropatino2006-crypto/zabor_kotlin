...
@Component({
  selector: 'app-sales-by-dinein',
  templateUrl: './sales-by-dinein.component.html',
  styleUrls: ['./sales-by-dinein.component.scss']
})
export class SalesByDineinComponent implements OnInit {

  restaurantList: any = [];
  employeeList: any = [];
  @Input() resId: Number = -1;
  @Input() delivery_mode: Number = 3;
  ...
  rerender(): void {
    ...
    this.dtTrigger.next();
    if (!this.lastWeek && !this.last30days && this.resId !== -1 && this.selectedDateRange !== null) {
  ...
  inputChanged(evt: Object) {
    ...
    //   this.mysqlStartDate = this.selectedDate.startDate.format("YYYY-MM-DD");
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        if (this.selectedDateRange != null && this.selectedDateRange.startDate && this.selectedDateRange.endDate) {
          console.log("coming inside this.selectedDateRange", this.selectedDateRange);
          dataTablesParameters['startDate'] = moment(this.selectedDateRange.startDate).format('DD-MM-YYYY HH:mm:ss');
          dataTablesParameters['endDate'] = moment(this.selectedDateRange.endDate).format('DD-MM-YYYY HH:mm:ss');
          this.tableTitle = `Report for date-range = ${moment(this.selectedDateRange.startDate).format('DD-MM-YYYY HH:mm:ss')} -- ${moment(this.selectedDateRange.endDate).format('DD-MM-YYYY HH:mm:ss')}`;
        }
