...
export class PaymentTestComponent implements OnInit, AfterViewInit {
  ...
  constructor(private _router: Router, private formBuilder: FormBuilder, private spinner: NgxSpinnerService) {
  ...
  ngAfterViewInit(): void {
    ...
    this.paymentMgr.getLocations().then(() => {
      this.usingSimulator = passedReader.id === 'SIMULATOR';
      this.paymentMgr.discoverReaders(this.usingSimulator ? 'st_simulated' : this.currentLocation.id, this.usingSimulator).then((discoverResult) => {
        if (discoverResult.success) {
          let tempReader: Reader | null = null;
          for (const discoveredReader of this.paymentMgr.discoveredReaders) {
            if (discoveredReader.id === paymentData.reader.id) {
