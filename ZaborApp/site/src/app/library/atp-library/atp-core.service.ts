...
export class AtpCoreService {
   ...
   * @experimental
   */
  public CalcDegrees(ele: any, parentPos: any, step: number): number {
    const clock = {
    ...
    const Vx = Math.round((ele.clientX - parentPos.left) - targetX);
    const Vy = Math.round(targetY - (ele.clientY - parentPos.top));
