...
@Component({
  selector: 'app-sales-by-delivery',
    templateUrl: './sales-by-delivery.component.html',
    styleUrls: ['./sales-by-delivery.component.scss']
})
export class SalesByDeliveryComponent implements OnInit {

  restaurantList: any = [];
  employeeList: any = [];
  shiftList: any = [];
  @Input() resId: Number = -1;
  @Input() delivery_mode: Number = 1;
  @Input() shiftId: Number = -1;
  tableTitle = 'Report (Delivery)';
  currentShift: any;
  currentEmployee: any;
  ...
  rerender(): void {
    ...
    if (!this.lastWeek && !this.last30days) {
      this.tableTitle = `Report (Delivery)`;
    }
  }
